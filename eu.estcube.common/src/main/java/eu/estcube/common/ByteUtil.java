...
public class ByteUtil {
    ...
    public static String toHexString(byte[] bytes, int limit) {
        ...
        int numberOfChars = numberOfBytes * 3 - 1 + (numberOfBytes < bytes.length ? 4 : 0);
        char[] hexChars = new char[Math.max(0, numberOfChars)];
        char[] tmp;
    ...
    public static byte[] toBytesFromHexString(String hexDump) {
        if (StringUtils.isBlank(hexDump)) {
            throw new IllegalArgumentException("Hex dump is empty or null");
        }
