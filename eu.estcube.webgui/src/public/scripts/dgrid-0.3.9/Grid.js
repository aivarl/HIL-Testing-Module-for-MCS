				...
				if(event.type == "click" || event.keyCode == 32 /* space bar */ || (!has("opera") && event.keyCode == 13) /* enter */){
					...
					do{
						if(target.sortable){
							...
							var column = columns[sortArray[0].attribute];
							if (column.sortBy) {
								newSort = function(a, b) {
	        						if(a === undefined){ a = ""; } else { a = column.sortBy(a); }
	        						if(b === undefined){ b = ""; } else { b = column.sortBy(b); }
	        						return a == b ? 0 : (a > b == !sortArray[0].descending ? 1 : -1);
								};
							} else {
								newSort = sortArray;
							}
							...
							// update sort arrow in case newSort is function
							grid.updateSortArrow(sortArray, true);
