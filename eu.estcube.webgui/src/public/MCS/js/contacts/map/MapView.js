define([
    ...
    function(declare, Lang, Arrays, DomConstruct, Dom, on, BorderContainer, ContentPane, TitlePane,
            ExpandoPane, CheckBox, config, MissionInformationStore, OrbitalStateStore,
            ForEachStoreElement, ObservableContentPane) {
        ...
        return declare([], {
            constructor: function(args) {
                ...
                var traceArrowLayer = new OpenLayers.Layer.Vector("Orbital Trace Direction Arrows Layer", {
                   styleMap: new OpenLayers.StyleMap(config.MAP.styles.traceArrowLayer),
                   //renderes: renderer
                });
                ...
                var radioCoverageLayer = new OpenLayers.Layer.Vector("Radio Coverage Layer", {
                    styleMap: new OpenLayers.StyleMap(config.MAP.styles.radioCoverageLayer)
                });
                ...
                //map.setCenter( new OpenLayers.LonLat(0.0, 0.0), 2 ); // Not needed as the zoom is set in the PlaceAt function
                ...
                // For each GS we add a point on the map and toggle buttons to UI
                ...
                // Note: If state at a different time will be needed, change Date.now() calls to some function, that
                // returns the timestamp with some delta

                // Main function. For a satellite with ID <satelliteId>, sets up tracking structures and
                // event listeners that will move the sat on the map according to the received (possibly
                // incomplete) data points.
                function SetupUpdater(satelliteId, satelliteName) {
                    // Priority queue (PQ), where all the known future positions are held, sorted
                    // by timestamp. The next known position (NP) is not contained here, but in
                    ...
                    //      LKP.timestamp < Date.now() < NKP.timestamp < futurePositions[0].timestamp
                    //
                    function ProcessMessage(message) {
                        ...
                        if(trackingData.pos != null) { // If we already have some known location in past/present
                            if(msg.timestamp > trackingData.pos.timestamp) {
                                if(msg.timestamp > now) {
                                    if(trackingData.nextPos != null && msg.timestamp > trackingData.nextPos.timestamp) { // Future point after NP
                            ...
                            // TODO: noticed the bug too late, this is the easiest way to fix, but there should be a way to refactor it later
                            if(msg.timestamp > now) {
                                // Message is in future - can't set is as LKP, just add it to the PQ
                                var index;
                                for(index = 0; index < futurePositions.length && futurePositions[index].timestamp < msg.timestamp; index++);
                                ...
                                trackingData.feature.attributes = { label: satelliteName }; // attributes can be reference by feature stylemap

                                satelliteLocationLayer.addFeatures([trackingData.feature]);

                                if(futurePositions.length > 0) { // If we already know some future points, set NP
                                    UpdateNearestPoint();
                                }
                            }
                        }
                    ...
                    ForEachStoreElement(OrbitalStateStore.query({satelliteId : satelliteId}), Lang.hitch(this, function(msg) {
                        ProcessMessage(msg);
                    }));
                    ...
                    function UpdateInterpolation() {
                        ...
                        if(data.pos != null && data.nextPos != null) {
                            // If there is a map boundary between LKP and NP, do no interpolation. This is needed to avoid
                            // interpolating the position through the whole screen - engine knows nothing about the Earth being
                            // a spheroid. In future, proper solution would be to add fictive point on the edges on both LKP
                            ...
                            if(!IsWrap(data.pos, data.nextPos)) {
                                ...
                                // TODO: The UpdateFeature function shouldn't need any input arguments
                    ...
                    function CreateTrace() {
                        ...
                        //
                        // Constructor is one big sideeffect - adds a corresponding feature to the layer
                        function Segment(segmentPoints, inSunlight) {
                            ...
                            var geometry = new OpenLayers.Geometry.LineString(Arrays.map(this.points, function(point) {
                                return new OpenLayers.Geometry.Point(point.lon, point.lat)
                            }));

                            this.feature = new OpenLayers.Feature.Vector(geometry, {
                                orbitColor: (inSunlight ? config.MAP.styles.orbitColorInSunlight : config.MAP.styles.orbitColorNotInSunlight)
                            });
                        ...
                        // Splits the points into the biggest possible segments. Split points
                        ...
                        function CreateInitialSegments(points) {
                            ...
                            for(var i = 1; i < points.length; i++) {
                                ...
                                if(lightChange || wrap) {
                                    // Split. If the split is due to sunlight change, add and extra point to
                        ...
                        // Delete old trace
                        orbitalTraceLayer.destroyFeatures(Arrays.map(traceFeatures[satelliteId], function(segment) {
                            return segment.feature;
                        }));

                        // Create a new one
                        var points = OrbitalStateStore.query({ satelliteId: satelliteId }, {
                            sort: [{ attribute: 'timestamp', descending: false}]
                        });

                        var segments = CreateInitialSegments(Arrays.map(points, function(p) {
                            return MessageToPosition(p);
                        }));
                    ...
                    function createRadioCoverageFeature(lat, lon, alt) {
                        ...
                        //srcPoint.lat = -62; srcPoint.lon = -170; srcPoint.alt = 1000*1000;
                        ...
                        //       At the moment it's only a straight average of neighbouring points

                        for(i = 0; i <= 360; i += config.MAP.radioCoverageBoundaryStepSize) {
                            var newPoint = srcPoint.pointAtBearingDist(i, radius);

                            // Check for 180/-180 longitude wrapping
                            // and create additional points for correct drawing if needed
                    ...
                    // For each batch update to the store, recreate the trace. This is easier than
                    // trying to maintain the trace structure with each new data point arrival
                    OrbitalStateStore.onUpdate(CreateTrace);
                ...
                this.createUI(groundStationLocationLayer, this.gsFeatures, satelliteLocationLayer,
                        satellitesTracked);
            },

            // Adds missing GS/sat entries in the control panel. Needed because GS/sat info is received
            // dynamically from the server
            updateUI: function() {
                if(this.uiInit) { // otherwise, the widget will be created anyway in createUI()
                    Arrays.forEach(this.groundStations, function(gs) {
                        if(!(gs.ID in this.drawnIDs)) {
                            this.createCheckbox(gs, this.gsListPane, Lang.hitch(this, function(value, id) {
                                if(value) {
                                    this.gsLayer.addFeatures([this.gsFeatures[id]]);
                                } else {
                                    this.gsLayer.removeFeatures([this.gsFeatures[id]]);
                                }
                            }));
                        }
                    }, this);

                    Arrays.forEach(this.satellites, function(sat) {
                        if(!(sat.ID in this.drawnIDs)) {
                            this.createCheckbox(sat, this.satListPane, Lang.hitch(this, function(value, id) {
                                this.satellitesTracked[id].showing = value;

                                var trace = Arrays.map(this.traceFeatures[id], function(segment) {
                                    return segment.feature;
                                });

                                var arrows = Arrays.map(this.traceFeatures[id], function(segment) {
                                    return segment.arrowFeature;
                                });
            ...
            createCheckbox: function(obj, parent, callback) {
                var cbPane = new ContentPane();

                var checkBox = new CheckBox({id : (obj.ID + "-cb"), checked : true})
                checkBox.onChange = function(value) {
                    callback(value, obj.ID);
                }

                cbPane.addChild(checkBox);
                cbPane.domNode.appendChild(DomConstruct.create("label", {"for" : (obj.ID + "-cb"), innerHTML : obj.name}));

                parent.addChild(cbPane);

                this.drawnIDs[obj.ID] = true;
            },

            createUI: function(gsLayer, gsFeatures, satLocationLayer, satellitesTracked) {
                this.mainContainer = DomConstruct.create("div", {style: {width: "100%", height: "100%"}});

                this.layout = new BorderContainer({}, this.mainContainer);
                this.mapPane = new ObservableContentPane({ region: "center" });
                on(this.mapPane, "resized", Lang.hitch(this, function() {
                    this.map.updateSize();
                }));
                ...
                this.gsListPane = new TitlePane({id : "gslist", title: "Ground stations", style : {"margin-bottom" : "10px"}});
                this.satListPane = new TitlePane({id : "satlist", title: "Satellites", style : {}});
                listPane.addChild(this.gsListPane);
                listPane.addChild(this.satListPane);
            ...
            placeAt: function(container) {
                ...
                this.map.setOptions({
                    restrictedExtent: this.map.getMaxExtent()
                });
            },
