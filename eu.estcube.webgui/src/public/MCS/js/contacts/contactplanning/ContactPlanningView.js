define(['dojo/_base/declare', 
    ...
    function (declare, Lang, Arrays, connect, dom, DomConstruct, on, keys, Gfx, ExpandoPane, TableContainer,
        ...
        var conf = config.CONTACTPLANNING;

        return declare([], {
            constructor: function(args) {
                this.radarView  = new RadarViewContentProvider({id:"plan"});
                this.mainContainer = DomConstruct.create('div', { id: "planMainContainer" });
                this.plotContainer = DomConstruct.create('div', { id: "planPlotContainer", style: {overflow: "hidden"} }, this.mainContainer);
                ...
                this.tableContainer = new dojox.layout.TableContainer({
                	cols: 2,
                    id: 'contactinfotable',
                    orientation: "vert",
                    showLabels: false,
               	...
               	this.tooltipContent = new ContentPane({
                    content:"<strong>Contact data:</strong><div id=\"tooltipContent\"></div>"
                  });
               	this.tooltipContent.startup();
               	

                this.radarViewPane = new ContentPane({
                    content: this.radarView.getContent(),
                    style: { 'width': '20vh', 'height': '25vh', 'overflow': 'hidden'}
                });


               	this.tableContainer.addChild(this.radarViewPane);
               	this.tableContainer.addChild(this.tooltipContent);
				this.infoPane.addChild(this.tableContainer);
				
                this.layout.addChild(this.plotPane);
                this.layout.addChild(this.infoPane);
                ...
                setInterval(ReplotIfVisible, config.CONTACTPLANNING.updateInterval);
